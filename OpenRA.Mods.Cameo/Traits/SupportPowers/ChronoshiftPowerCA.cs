...
{
	class ChronoshiftPowerCAInfo : SupportPowerInfo
	{
		...
		[Desc("Ticks before teleportation.")]
	...
	class ChronoshiftPower : SupportPower, ITick
	{
		bool activated;
		int ticks;
		CPos from, to;

		public ChronoshiftPower(Actor self, ChronoshiftPowerCAInfo info)
			: base(self, info) { }
		...
		{
			var range = ((ChronoshiftPowerCAInfo)Info).Range;
		...
		void ITick.Tick(Actor self)
		{
			...
			var targetDelta = to - from;
			foreach (var target in UnitsInRange(from))
			{
				var chronoshiftable = target.TraitsImplementing<Chronoshiftable>()
					.FirstEnabledTraitOrDefault();

				if (chronoshiftable == null)
					continue;

				var targetCell = target.Location + targetDelta;

				if (self.Owner.Shroud.IsExplored(targetCell) && chronoshiftable.CanChronoshiftTo(target, targetCell))
					chronoshiftable.Teleport(target, targetCell, info.Duration, info.KillCargo, self);
			}
