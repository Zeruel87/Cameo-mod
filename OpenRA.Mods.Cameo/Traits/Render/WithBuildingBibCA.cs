...
{
	public class WithBuildingBibCAInfo : ConditionalTraitInfo, Requires<BuildingInfo>, IRenderActorPreviewSpritesInfo, IActorPreviewInitInfo, Requires<RenderSpritesInfo>
	{
		[SequenceReference]
		...
		public override object Create(ActorInitializer init) { return new WithBuildingBibCA(init.Self, this); }
	...
	public class WithBuildingBibCA : ConditionalTrait<WithBuildingBibCAInfo>, INotifyAddedToWorld, INotifyRemovedFromWorld
	{
		readonly WithBuildingBibCAInfo info;
		readonly RenderSprites rs;
		readonly BuildingInfo bi;
		readonly List<AnimationWithOffset> anims = new();

		public WithBuildingBibCA(Actor self, WithBuildingBibCAInfo info)
			: base(info)
		{
			this.info = info;
		...
		void INotifyAddedToWorld.AddedToWorld(Actor self)
		{
			...
			for (var i = 0; i < rows * width; i++)
			{
				...
				var awo = new AnimationWithOffset(anim,
					() => offset,
					() => IsTraitDisabled,
					-(offset.Y + centerOffset.Y + 512));
