...
{
	public class DropPodsPowerCAInfo : SupportPowerInfo, IRulesetLoaded
	{
		[FieldLoader.Require]
		[Desc("Drop pod unit")]
		...
		[Desc("If true, drop the exact units listed rather than randomly selecting them")]
		public readonly bool ExactUnits = false;
		...
		public override object Create(ActorInitializer init) { return new DropPodsPowerCA(init.Self, this); }
	...
	public class DropPodsPowerCA : SupportPower
	{
		readonly DropPodsPowerCAInfo info;

		public DropPodsPowerCA(Actor self, DropPodsPowerCAInfo info)
			: base(self, info)
		{
			this.info = info;
		}
		...
		public void SendDropPods(Actor self, Order order, WAngle facing)
		{
			...
			self.World.AddFrameEndTask(w =>
			{
				...
				var dropUnits = new List<string>();

				// CA alteration to use UnitTypes as an explicit list of units
				if (info.ExactUnits)
				{
					foreach (var unitType in info.UnitTypes)
						dropUnits.Add(unitType);
				}
				else
				{
					...
					for (var i = 0; i < drops; i++)
						dropUnits.Add(info.UnitTypes.Random(self.World.SharedRandom));
				}

				foreach (var unitType in dropUnits)
				{
