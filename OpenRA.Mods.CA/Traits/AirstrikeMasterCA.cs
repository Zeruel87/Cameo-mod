...
namespace OpenRA.Mods.CA.Traits
{
	...
	public class AirstrikeMasterCA : BaseSpawnerMaster, ITick, INotifyAttack, IResolveOrder
	{
			...
			foreach (var se in SlaveEntries)
			{
				...
				if (AirstrikeMasterInfo.SpawnDistance > WDist.Zero && slaveEntry.IsValid && slaveEntry.IsLaunched && slaveEntry.Actor.CurrentActivity is ReturnAirstrikeMaster)
				{
					...
					if (dist > AirstrikeMasterInfo.SpawnDistance.Length)
					{
						...
						respawnTicks = Util.ApplyPercentageModifiers(Info.RespawnTicks, reloadModifiers.Select(rm => rm.GetReloadModifier(null)));
					}
				}
			}
		}
		...
		{
			if (AirstrikeMasterInfo.CancelOnStop && (order.Target != lastTarget))
				Recall();
		}
